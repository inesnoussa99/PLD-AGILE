...
class Adresse(SQLModel, table=True):
    ...
    depart_programmes: List["Programme"] = Relationship(back_populates="adresse_depart")

class Client(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    nom: str
    tel: str
    adresse_id: str = Field(foreign_key="adresse.id")
...
class Programme(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    date_depart: time
    adresse_depart_id: str = Field(foreign_key="adresse.id")
...
class Livraison(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    adresse_pickup_id: str = Field(foreign_key="adresse.id")
    adresse_delivery_id: str = Field(foreign_key="adresse.id")
    duree_pickup: int
    duree_delivery: int
    date: date
    programme_id: Optional[int] = Field(default=None, foreign_key="programme.id")

    # Précision des foreign_keys nécessaire car il y a 2 liens vers Adresse
...
# Forward reference update for Etape
...
class Troncon(SQLModel, table=True):
    ...
    destination_id : str = Field(foreign_key="adresse.id")
    longueur: float
    origine_id : str = Field(foreign_key="adresse.id")
    nomRue : str 
    
    origine: Adresse = Relationship(sa_relationship_kwargs={"foreign_keys": "Troncon.origine_id"})
    destination: Adresse = Relationship(sa_relationship_kwargs={"foreign_keys": "Troncon.destination_id"})
